...
5.  Add `chat:write` OAuth Scope to "Bot Token Scopes".
	1.  (optional) add `files:write` OAuth Scope if you will be uploading files.
	2.  (optional) add `users:read` and `users:read.email` OAuth Scope if you will be looking users up by email (using slackUserIdFromEmail(...) or slackUserIdsFromCommiters() methods)
